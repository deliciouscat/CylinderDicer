...
export interface LadderMatchCandidate {
	joinedAt: number
	mmr: number
}

export interface LadderMatchDecision {
	shouldStart: boolean
	playerCount: number
	reason: 'full' | 'projected_slow_fill' | 'max_wait' | 'waiting'
	mmrBand: number
...
export function ladderMmrBand(waitMs: number): number {
	...
	return Math.round(
		LADDER_INITIAL_MMR_BAND
			+ (LADDER_MAX_MMR_BAND - LADDER_INITIAL_MMR_BAND) * progress,
	)
}

export function estimateLadderArrivalRate(joinedAt: number[]): number {
	if (joinedAt.length < 2) return 0
...
export function decideLadderMatch(
	candidates: LadderMatchCandidate[],
	now: number,
): LadderMatchDecision {
	...
	if (
		playerCount >= LADDER_MIN_PLAYER_COUNT
...
export function eligibleLadderCandidates<T extends LadderMatchCandidate>(
	candidates: T[],
	now: number,
): T[] {
	const ordered = [...candidates].sort((left, right) => left.joinedAt - right.joinedAt)
