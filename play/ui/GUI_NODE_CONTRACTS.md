...
- 결투 중간의 판정/roulette step/hit-miss 텍스트는 표시하지 않는다. 결과는 일러스트, HP/탄환 indicator, hit flash와 오디오로 전달한다.
- `hit_flash`: 명중 이펙트 placeholder.
- `player_dice_template`, `grid_dice_template`, `tray_dice_template`: 반복 주사위 runtime clone용 template. 에디터에 65개 반복 노드를 직접 두지 않는다. 컵 아래 `player_dice_template` clone은 shake 결과와 같은 `a1`–`a5` table angle을 사용하고, 로컬 공개 주사위는 하단 `hint` 안내문구를 가리지 않도록 상대 좌석보다 위에 배치한다. 판독용 `grid_dice_template`/`tray_dice_template` clone은 정면 `a0`를 사용하고 각각 `grid_panel`, `tray` 좌표계에 남긴다.

## result.gui

- `root`: dim 배경과 결과 입력을 소유하는 전체 overlay.
- `plaque`: `rank_result` atlas의 `rank_1`, `rank_2_3`, `rank_4_6`. 1위 금색 승리 panel, 2–3위 은색, 4–6위 동색 panel이다.
...
- `lobby_button`, `lobby_label`: `EXIT_TO_LOBBY`를 한 번만 emit한다.
- `spectate_button`, `spectate_label`: match 진행 중 탈락 HUD에서만 표시하며 local `ui.spectating`을 켜 overlay를 닫는다.
