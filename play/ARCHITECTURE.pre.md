...
flowchart TD
    ...
    Main["main/main.script"] -->|"SUBMIT_MATCH_RESULT"| Bridge
...
렌더링 방식: **하이브리드 권장** — 배경 파노라마는 GO+sprite(카메라), 화면 좌석에 종속되는 캐릭터·컵·주사위·HUD는 GUI로 렌더링한다. `player_carousel`과 `shake.gui`는 `ui/common/table_seat_layout.lua`의 반원 좌석 배치를 공유하며, 컵은 `shake.gui`가 소유한다.

## 5. 모듈별 책임과 상호작용 (문서의 핵심 표)
- `main/main.script`(리팩터): 부트스트랩/composition root. 브릿지 install, store 생성, director 마운트, 입력 포커스 초기화, 매치 종료 시 결과를 한 번만 제출한다. ↔ `game_bridge`, `match_adapter`, `store`, `director`.
- `game/director.script`: 턴 FSM 구동, 활성 블럭 교체(`BlockController`), `bgAnimate` 동기화, `cylinderTarget()` 앵커 결정. setup/bidding은 파노라마 상단, shaking/dualing은 테이블 하단으로 배경을 이동시킨다. store callback에서는 sync 요청만 기록하고 실제 Defold message는 lifecycle `update()`에서 보낸다. ↔ `turn_machine`, `event_bus`, `anchors`, `tween`, 모든 `ui/*`, `background`.
...
- `core/cosmetics.lua`: skinId→atlas/이미지 해석(dice/cup/revolver/bg/rail), Live Update 대비. 현재 cup GUI는 컴파일된 `props_default.atlas`를 사용하고, 추가 스킨 atlas 등록 시 이 설정을 따른다. ↔ View, `match_adapter`.
