...
def `결투집행`상세:
    ...
    판정 시 face `1` 해골은 특수 눈이다. 콜한 눈이 해골이 아니면 해골도 콜한 눈 집계에 포함한다. 콜한 눈이 해골이면 해골만 집계하고, 결투 판정에 사용하는 요구 수량은 rail에 표시된 콜 수량의 절반(홀수는 내림)이다. 예: Skull 7개 콜은 실제 Skull 3개를 기준으로 부족/초과/정확과 격발 횟수를 계산한다. rail의 콜 수량과 입찰 순서 자체는 바뀌지 않는다.
    장전 없음.
